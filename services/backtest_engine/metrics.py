...
class PerformanceMetrics:
    ...
    @staticmethod
    def calculate_average_gain_loss(trades: List[Dict[str, Any]]) -> Dict[str, float]:
        """
        ...
        return {
            'avg_gain': avg_gain,
            'avg_loss': avg_loss,
            'profit_factor': round(avg_gain / avg_loss, 2) if avg_loss > 0 else 0.0
        }
    
    @staticmethod
    def compare_vs_buy_hold(strategy_return: float, buy_hold_return: float) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def generate_report(portfolio_values: List[float], trades: List[Dict[str, Any]], 
                       returns: pd.Series, buy_hold_return: float = None) -> Dict[str, Any]:
        """
        ...
        if buy_hold_return is not None:
            comparison = PerformanceMetrics.compare_vs_buy_hold(total_return, buy_hold_return)
            report['vs_buy_hold'] = comparison
        
        logger.info("Generated performance report", 
                   total_return=total_return, sharpe=sharpe, trades=len(trades))
