...
class MarketDataConnector:
    ...
    def get_ohlcv(self, ticker: str, period: str = "1mo") -> dict:
        ...
        """
        try:
            logger.info(f"Fetching data for {ticker}", ticker=ticker, period=period)
            
            stock = yf.Ticker(ticker)
            df = stock.history(period=period)
            
            if df.empty:
                raise DataFetchError(f"No data found for {ticker}", ticker=ticker)
    ...
    def get_multiple_tickers(self, tickers: list, period: str = "1mo") -> dict:
        ...
        for ticker in tickers:
            try:
                ...
                logger.warning(f"Failed to fetch {ticker}", ticker=ticker, error=str(e))
