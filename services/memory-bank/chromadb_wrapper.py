...
class MemoryBank:
    ...
    def store_risk_profile(self, user_id: str, risk_tolerance: str, max_drawdown: float, 
                          preferred_sectors: list, metadata: dict = None):
        """
        ...
        """
        try:
            content = f"Risk Tolerance: {risk_tolerance}, Max Drawdown: {max_drawdown}%, Sectors: {', '.join(preferred_sectors)}"
            ...
            logger.info("Stored risk profile", user_id=user_id, risk_tolerance=risk_tolerance)
    ...
    def update_preference_from_feedback(self, user_id: str, feedback_type: str, content: str):
        """
        ...
        try:
            ...
            logger.info("Stored user feedback", user_id=user_id, feedback_type=feedback_type)
    ...
    def store_agent_output(self, agent_name: str, ticker: str, output: str, 
        ...
        try:
            ...
            logger.info("Stored agent output", agent_name=agent_name, ticker=ticker)
            
        except Exception as e:
            raise MemoryBankError(f"Failed to store agent output: {str(e)}")
    
    def retrieve_similar_analysis(self, ticker: str, timeframe_days: int = 30, n_results: int = 5):
        """
        ...
        try:
            ...
            logger.info("Retrieved similar analysis", ticker=ticker, 
                       results_count=len(results["documents"][0]) if results["documents"] else 0)
            
            return results
            
        except Exception as e:
            raise MemoryBankError(f"Failed to retrieve similar analysis: {str(e)}")
    ...
    def get_accuracy_metrics(self, agent_name: str, ticker: str = None):
        ...
        try:
            ...
            confidences = [m.get("confidence", 0) for m in results["metadatas"][0]]
    ...
    def compact_old_data(self, days_to_keep: int = 90):
        ...
        try:
            ...
            logger.info("Compact old data called", days_to_keep=days_to_keep, 
                       cutoff_date=cutoff_date,
                       note="Archival not fully implemented - ChromaDB limitation")
            
            return {"status": "partial", "message": "Archival logged, full implementation pending"}
